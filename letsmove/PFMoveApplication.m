...
#define kStrMoveApplicationNotifyTitle  _I10NS(@"Move to Applications folder")
#define kStrMoveApplicationQuestionMessage _I10NS(@"I can move myself to the Applications folder if you'd like.")
#define kStrMoveApplicationNotifyMessage _I10NS(@"I need to move myself to the Applications folder in order to receive future updates on this version of macOS.")
...
void PFMoveToApplicationsFolderIfNecessary(void) {
  ...
  // macOS 10.12 introduced gatekeeper path randomization that means we really
  // need users to move the app into Applications for Sparkle to function.
  BOOL macOS1012OrNewer = kCFCoreFoundationVersionNumber > kCFCoreFoundationVersionNumber10_11_Max;
    ...
    if (macOS1012OrNewer) {
      alert.messageText = kStrMoveApplicationNotifyTitle;
    } else if (installToUserApplications) {
      alert.messageText = kStrMoveApplicationQuestionTitleHome;
    } else {
      alert.messageText = kStrMoveApplicationQuestionTitle;
    }

    if (macOS1012OrNewer) {
      informativeText = kStrMoveApplicationNotifyMessage;
    } else {
